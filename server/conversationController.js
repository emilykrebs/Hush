    ...
    Conversation.find({ participants: { name: sender } }) //returns an array with all convos matt has
      .then((allConvosWithSender) => {
        if (allConvosWithSender.length === 0) {
          //the sender has no convos with anyone
          Conversation.create({
            _id: mongoose.Types.ObjectId(),
            participants: [{ name: sender }, { name: recipient }],
            messages: [],
          }).then((mongoResult) => {
        ...
        for (
          let indivConvo = 0;
          indivConvo < allConvosWithSender.length;
          indivConvo++
        ) {
          for (
            let convoParticipants = 0;
            convoParticipants <
            allConvosWithSender[indivConvo].participants.length;
            convoParticipants++
          ) {
            let currentRecipient =
              allConvosWithSender[indivConvo].participants[convoParticipants]
                .name;
